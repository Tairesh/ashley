...
class Database:
    ...
    SQL_CREATE_LEMONS_INDEX = (
        """CREATE UNIQUE INDEX IF NOT EXISTS lemons_image_index ON lemons (image)"""
    )
    ...
    SQL_USER_GET = (
        "SELECT user_id, first_name, last_name, username, language, status, date_time "
    ...
    SQL_GET_CHATS_SETTINGS = (
        "SELECT chat_id, language, enabled_porn, enabled_anime, enabled_replies "
        "FROM chats_settings WHERE chat_id = ?"
    )
    SQL_CHAT_SETTINGS_ADD = (
        "INSERT INTO chats_settings (chat_id, language) VALUES (?, ?)"
    )
    SQL_CHAT_SETTINGS_UPDATE = (
        "UPDATE chats_settings SET enabled_porn = ?, enabled_anime = ?, enabled_replies = ? "
        "WHERE chat_id = ?"
    )
    SQL_LEMONS_ADD = (
        "INSERT INTO lemons (image, owner_id) VALUES (?, NULL) ON CONFLICT DO NOTHING"
    )
    SQL_LEMONS_GET_FREE = (
        "SELECT id, image, owner_id FROM lemons WHERE owner_id IS NULL"
    )
    SQL_LEMONS_GET = (
        "SELECT id, image, owner_id FROM lemons WHERE owner_id = ? ORDER BY id"
    )
    ...
    def save_user_and_chat(self, user, chat):
        ...
        if chat and chat.id != user.id:
            ...
            if user.id not in chat.users:
                ...
                cur.execute(
                    self.SQL_CHAT_UPDATE_USERS,
                    [",".join((str(u) for u in chat.users)), chat_id],
                )
                con.commit()
    ...
    def update_chat_settings(
        self, chat_id, enabled_porn=1, enabled_anime=1, enabled_replies=1
    ):
        con = sqlite3.connect(self._db_path)
        cur = con.cursor()
        cur.execute(
            self.SQL_CHAT_SETTINGS_UPDATE,
            (enabled_porn, enabled_anime, enabled_replies, chat_id),
        )
        con.commit()
        con.close()
