...
class Weather(Action):
    ...
    def after_loaded(self):
        Weather.OPENWEATHERMAP_API = Weather.OPENWEATHERMAP_API.replace(
            "%APPID%", self.tgb.api_keys["openweathermap_appid"]
        )
    ...
    def call(self, message: Message):
        ...
        data = json.loads(
            requests.get(self.OPENWEATHERMAP_API.format(quote(text))).content.decode(
                "utf-8"
            )
        )
        if "cod" in data and data["cod"] == 200:
            ...
            self.bot.reply_to(
                message, f"{emoji.SAD} Не могу найти такое место «{text}»"
            )
        else:
