...
class Youtube(Action):
    ...
    def after_loaded(self):
        Youtube.API_URL = Youtube.API_URL.replace(
            "%KEY%", self.tgb.api_keys["google_apikey"]
        )
        Youtube.API_URL = Youtube.API_URL.replace(
            "%CX%", self.tgb.api_keys["google_cx"]
        )
    ...
    def call(self, message: Message):
        ...
        data = json.loads(
            requests.get(self.API_URL.format(quote(keyword))).content.decode("utf-8")
        )
        if "items" in data:
