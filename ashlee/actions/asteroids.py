...
class Asteroids(Action):
    ...
    def call(self, message: Message):
        ...
        data = json.loads(
            requests.get(self.API_URL.format(now, then)).content.decode("utf-8")
        )
        asteroids = []
        for day in data["near_earth_objects"]:
            for row in data["near_earth_objects"][day]:
                ...
                size_min = row["estimated_diameter"]["kilometers"][
                    "estimated_diameter_min"
                ]
                if size_min < 50:
                    size_min = row["estimated_diameter"]["meters"][
                        "estimated_diameter_min"
                    ]
                    size_max = row["estimated_diameter"]["meters"][
                        "estimated_diameter_max"
                    ]
                    size_metrics = "метров"
                else:
                    size_max = row["estimated_diameter"]["kilometers"][
                        "estimated_diameter_max"
                    ]
                ...
                approach_delta = (
                    datetime.datetime.fromtimestamp(approach // 1000)
                    - datetime.datetime.now()
                )
                approach_distance = round(
                    float(row["close_approach_data"][0]["miss_distance"]["kilometers"])
                )
                approach_velocity = round(
                    float(
                        row["close_approach_data"][0]["relative_velocity"][
        ...
        if len(asteroids) == 0:
            self.bot.reply_to(
                message,
                f"В ближайшие сутки никаких астероидов рядом с Землёй не пролетит! "
        ...
        for row in asteroids:
            ...
            text += (
                f"{emoji.WARNING if hazard else emoji.SPACE} <b><a href='{url}'>{name}</a></b> "
