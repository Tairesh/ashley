...
class Lemons(Action):
    ...
    def call(self, message):
        ...
        lemons = [
            f"{emoji.LEMON} LMN #{lemon.id}"
            for lemon in self.db.get_user_lemons(message.from_user.id)
        ]
        count = len(lemons)
        if count == 0:
            ...
            self.bot.reply_to(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                f"{emoji.LEMON} Посмотреть все",
                                callback_data="lemons:view_all",
                            )
                        ]
                    ],
                    1,
    ...
    def btn_pressed(self, call: CallbackQuery):
        if call.data.endswith("view_all"):
            media = [
                InputMediaPhoto(
                    open(join(self.DIR, lemon.image), "rb"), f"LMN #{lemon.id}"
                )
