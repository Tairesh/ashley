...
class Readonly(Action):
    ...
    r_time_interval = re.compile(
        r"^(\d+)\s*(s|m|h|d|w|y|с|м|ч|д|н|л|)", flags=re.IGNORECASE
    )
        ...
        try:
            ...
            if keyword.strip() == "":
                ...
                if match:
                    ...
                    self.bot.reply_to(
                        message,
                        "Возможные значения: `5м` или `10 дней`",
                        parse_mode="Markdown",
                    )
                    return
            ...
            self.bot.reply_to(
                message, f"{utils.user_name(user)} получил ридонли на {interval} секунд"
            )
        except ApiException:
